...
/**
 * Betterpay Payment Gateway Class
 */
class Betterpay extends WC_Payment_Gateway {
	function __construct() {
		$this->id = "Betterpay";

		$this->method_title = __( "Betterpay", 'Betterpay' );

		$this->method_description = __( "Betterpay Payment Gateway Plug-in for WooCommerce", 'Betterpay' );

		$this->title = __( "Betterpay - Pay with internet banking (FPX) or credit / debit cards", 'Betterpay' );
		...
		add_action( 'woocommerce_api_'. strtolower( get_class($this) ), array( $this, 'check_Betterpay_response' ) );
		// add_action( 'woocommerce_api_callback', 'callback' );
	...
	public function init_form_fields() {
		$this->form_fields = array(
			'enabled'        => array(
				'title'   => __( 'Enable / Disable', 'Betterpay' ),
				'label'   => __( 'Enable this payment gateway', 'Betterpay' ),
				'type'    => 'checkbox',
				'default' => 'no',
			),
			...
			// ),
			'merchantId' => array(
				'title'    => __( 'Merchant ID', 'Betterpay' ),
				'type'     => 'text',
				'desc_tip' => __( 'This is the merchant ID that you can obtain from profile page in Betterpay', 'Betterpay' ),
			),
			'secretkey'      => array(
				'title'    => __( 'API Key', 'Betterpay'),
				'type'     => 'text',
				'desc_tip' => __( 'This is the API key that you can obtain from profile page in Betterpay profile', 'Betterpay' ),
			),
			'order_status' => array(
				 'title' => 'Order Status',
				 'desc_tip' => __( 'Order status to be set after the transaction is successfully paid.', 'Betterpay' ),
				 'type' => 'select',
				 'default' => '1',
				 'label' => 'Label', // checkbox only
				 'options' => array(
			...
			'title' => array(
				'title' => __( 'Title', 'Betterpay' ),
				'type' => 'text',
				'description' => __( 'This controls the title which the user sees during checkout.', 'Betterpay' ),
				'default' => __( 'Betterpay - Pay with Online Banking, Debit/Credit Card, e-Wallet or BNPL', 'Betterpay' ),
				'desc_tip'      => true,
				),
			'description' => array(
				'title' => __( 'Payment Description', 'Betterpay' ),
				'type' => 'textarea',
				'default' => ''
			)
		);
			...
			<div id="post-body" class="metabox-holder columns-2">
				...
				<div id="postbox-container-1" class="postbox-container">
					<div id="side-sortables" class="meta-box-sortables ui-sortable">
						<div class="postbox ">
							<h3 class="hndle"><span align="center">
							<img src="https://www.betterpay.me/merchant/assets/images/betterpay.png" width="150px;">
							<br>
							Fast and Automated Payments. Built for Merchants.</span></h3>
							<div class="inside">
								<div class="support-widget">
									<ul>
										<li>» Pay Bills & Collect Faster</li>
										<li>» Instant Settlements For Your Online Sales</li>
										<li>» A variety of payment methods with flexible rates from trusted financial institutions across the ASEAN region.</li>
									</ul>
									<a href="https://www.betterpay.me/" class="button wpruby_button" target="_blank">Register Now</a>
								</div>
							</div>
						</div>
					</div>
	...
	public function process_payment( $order_id ) {
		...
		return array(
			'result'   => 'success',
			'redirect' => $url.'/wc-api/Betterpay/?submit_data=true&order_id='.$order_id
		);
	}


	public function check_Betterpay_response() {

		$merchantId = sanitize_text_field($this->merchantId);

		$account_type = ((str_contains($merchantId, 'R')))?'2':'1';

		// $account_type = sanitize_text_field($this->account_type);
		$submit_data = isset($_GET['submit_data']) ? sanitize_text_field($_GET['submit_data']) : [];
		if(isset($submit_data) && !empty($submit_data) && $submit_data == 'true')
		{
			$this->submitData();
		...
		if(isset($_REQUEST['bp_lite_trx_id']) && $account_type === '2') {
			$this->bpLiteCallback();
		}
	}
	...
	public function submitData() {
		...
		# Prepare the data to send to Betterpay
		...
		// $account_type = sanitize_text_field($this->account_type);
		$account_type = ((str_contains($merchantId, 'R')))?'2':'1';
		$getcurrency = sanitize_text_field($old_wc ? $customer_order->currency : $customer_order->get_currency());

		$payment_desc = $product_name.' x '.$product_quantity;
		...
		$new_order_id = $merchantId."-".$order_id."-".date("his")."-".$randomString;
		...
		if($account_type !== '1') { 
			$callbackUrl = get_site_url() . '/?wc-api=betterpay';
			$post_args['baggage_variable'] = 'order_id|';
			$post_args['phone'] = $phone;
			$post_args['amount'] = $amount;
			$post_args['currency'] = $getcurrency;
			$post_args['payment_desc'] = $payment_desc;
			$post_args['sent_from'] = 'woocommerce';
			$post_args['callback_url_be'] = $callbackUrl."&order_id=$order_id&cart_id=$order_id";
			$post_args['callback_url_fe_fail'] = $callbackUrl."&order_id=$order_id&status=failed";
			$post_args['callback_url_fe_succ'] = $callbackUrl."&order_id=$order_id&status=success";
		}

		if($account_type === '1') {
			$hash_send = md5( $secretkey . $merchantId . $order_id . $amount . $detail );	
		} else {
			$generateHash = array_merge($prod, $post_args);
			$hash_send = $this->generateHash($generateHash, $secretkey);
			// $this->dd($generateHash, $hash_send);
		}
		
		$post_args['hash'] = $hash_send;
		...
		if($account_type === '1') {
			if($payment_gateway_status == '1') {
				// $this->dd('sd');
				echo "<form action='".esc_url("https://www.demo.betterpay.me/merchant/api/v2/woocommerce_receiver")."' id='submit_data' method='POST'>";
			} else {		
				echo "<form action='".esc_url("https://www.betterpay.me/merchant/api/v2/woocommerce_receiver")."' id='submit_data' method='POST'>";
			}
		} else {
			echo "<form action='".esc_url("https:///lite.betterpay.me/api/merchant/v1/checkout")."' id='submit_data' method='POST'>";
		}
...
// $this->dd($_REQUEST);
		...
		// {
			...
			if((strtolower($status_req) == 'transaction approved' || strtolower($status_req) == 'success' || strtolower($status_req) == 'settled') && $order != null)
			{
				error_reporting(0);
				...
				.tableBP tbody tr:nth-child(odd) {
	...
	public function callback() {
		...
		if ( $order && $order_id != 0 ) {
			...
			if ( $hash_value == sanitize_text_field($_REQUEST['hash']) ) {
				if ( sanitize_text_field($_REQUEST['msg']) == 'Transaction Approved' || sanitize_text_field($_REQUEST['msg']) == 'Success' || sanitize_text_field($_REQUEST['msg']) == 'settled' ) {
					// if ( strtolower( $order->get_status() ) == 'pending' || strtolower( $order->get_status() ) == 'processing' ) {
						...
						// if ( strtolower( $order->get_status() ) == 'pending' ) {
							...
							$order->add_order_note( 'Payment successfully made through Betterpay ('.sanitize_text_field($_REQUEST['pay_method']).'). Transaction reference is ' . sanitize_text_field($_REQUEST['cart_id']) );
					...
					if ( strtolower( $order->get_status() ) == 'pending' ) {
							...
							add_filter( 'the_content', 'Betterpay_payment_declined_msg' );
					}
				}
			} else {
				add_filter( 'the_content', 'Betterpay_hash_error_msg' );
			}
		}
	...
	public function bpLiteCallback() {
		global $woocommerce;
		//sanitize request
		$trx_id = sanitize_text_field($_REQUEST['bp_lite_trx_id']);
		$invoice = sanitize_text_field($_REQUEST['invoice_no']);
		$msg = sanitize_text_field($_REQUEST['msg']);
		$pay_method = sanitize_text_field($_REQUEST['pay_method']);
		$cart_id = sanitize_text_field($_REQUEST['order_id']);
		...
		# Check if the data sent is valid based on the hash value
		$secretkey = sanitize_text_field($this->secretkey);
		// $hash_value = md5( $secretkey . $trx_id . $invoice . $msg );
		$generateHash = array_filter(
			$_REQUEST,
			fn($key) => $key !== 'hash' && $key !== 'wc-api' && $key !== 'cart_id',
			ARRAY_FILTER_USE_KEY
		);
		$hash_value = $this->generateHash($generateHash, $secretkey);

		if($order && $order_id != 0 && $hash === $hash_value) {
			$successArray = [
			...
			if (in_array(strtolower($msg), $successArray)) {
				$order->payment_complete();
				$order->add_order_note( 'Payment successfully made through Betterpay ('.$pay_method.'). Transaction reference is ' . $cart_id );
				$order->add_order_note( 'Customer Notes: '.$order_notes);
				$order->update_status($this->order_status);
				echo 'OK - successful transaction';
				$woocommerce->cart->empty_cart();
			} else {
				if (strtolower( $order->get_status() ) == 'pending') {
			...
			echo 'OK - failed hash or order ID not exist';
			add_filter( 'the_content', 'Betterpay_hash_error_msg' );
			exit();
		}
	...
	public function is_valid_for_use() {
	 ...
	 // BP Lite hashing method
	 public function generateHash($data, $apiKey) {
		...
		$hashStr = str_replace(' ', '', $hashStr);

        return hash_hmac('sha256', $hashStr, $apiKey);
    }
